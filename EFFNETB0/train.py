"""
EfficientNetB0 Trainer
- Phase 1: frozen base, train custom head
- Phase 2: unfreeze last 20% for fine-tuning
...
IMG_SIZE = (224, 224)  # EfficientNetB0 standard input
BATCH_SIZE = 32
NUM_CLASSES = 4
...
PHASE2_EPOCHS = 25
PHASE2_LR = 1e-5
UNFREEZE_FROM_PCT = 0.80  # Unfreeze top 20%

# Regularization
DROPOUT_RATE = 0.3
L2_REG = 1e-4
LABEL_SMOOTHING = 0.1
USE_MIXUP = False  # Changed to False for stability
MIXUP_ALPHA = 0.2
...
def apply_mixup(ds, alpha=0.2):
    ...
    # Drop incomplete batches to avoid shape mismatches
    ds_full = ds.drop_remainder(True)
    
    def mixup(x1, y1):
        # Shuffle and take next batch
        ds2 = ds_full.shuffle(buffer_size=1000)
        x2, y2 = next(iter(ds2))
    ...
    ds_mixed = ds_full.map(mixup, num_parallel_calls=tf.data.AUTOTUNE)
    return ds_mixed
...
def train(train_dir, val_dir, output_dir):
    ...
    best_model_path = os.path.join(output_dir, 'best_efficientnetb0.keras')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train EfficientNetB0')
